...
function checkAnswer(answer) {
    if (questionEl[currentQ].answer == questionEl[currentQ].choices[answer.id]) {
        score += 5;
...
function renderQuestion() {
    questionEl.textContent = questionEl[currentQ].title;
    for (i = 0; i < answersEl.children.length; i++) {
        answersEl.children[i].children[0].textContent = `${(i + 1)}: ${questionEl[currentQ].choices[i]}`;
    }
}
